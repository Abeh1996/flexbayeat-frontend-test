// src/app/(auth)/auth/vendor/signup/page.tsx
"use client";
...
export default function VendorSignupPage() {
  ...
  useEffect(() => {
    if (step === "complete") {
      checkOtpVerifiedAction().then((verified) => {
        if (!verified) {
          router.replace("/auth/vendor/signup");
        }
      });
    }
  }, [step, router]);
  ...
  const goTo = useCallback(
    (nextStep: Step, params?: Record<string, string>) => {
      ...
      router.push(`/auth/vendor/signup?${query.toString()}`);
    },
    [router],
  );
        ...
        >
          ...
          {step === "complete" && (
            <SignUpDetailsForm
              ...
              contact={contact}
              method={method}
              redirectTo="/vendor/complete-registration"
            />
          )}
        </motion.div>
      </AnimatePresence>
