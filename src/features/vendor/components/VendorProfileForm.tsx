// src/features/Vendor/components/VendorProfileForm.tsx
'use client';
...
import { VendorProfilePayload, VendorProfile } from '../types';
...
// logo/documents are optional here — vendor already has them on file
...
interface VendorProfileFormProps {
  ...
  onSuccess: (payload: Partial<VendorProfilePayload>) => void;
  isLoading: boolean;
}

export function VendorProfileForm({ vendorProfile, onSuccess, isLoading }: VendorProfileFormProps) {
  const [query, setQuery] = useState(
    [vendorProfile.addressLine1, vendorProfile.city, vendorProfile.region]
      .filter(Boolean)
      .join(', ')
  );
  ...
  const {
    ...
    defaultValues: {
      ...
      latitude: vendorProfile.latitude ?? undefined,
      longitude: vendorProfile.longitude ?? undefined,
    },
  });
  ...
  const handleDocsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const merged = [...newDocFiles, ...files];
    setNewDocFiles(merged);
    setValue('documents', merged, { shouldValidate: true });
  };

  const removeNewDoc = (index: number) => {
    const updated = newDocFiles.filter((_, i) => i !== index);
    setNewDocFiles(updated);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        >
          {logoPreview ? (
            <>
              ...
              <img src={logoPreview} alt="New logo preview" className="w-14 h-14 object-cover border border-zinc-200" />
              <div>
                <p className="text-sm font-semibold text-zinc-700">New logo selected</p>
            ...
            <>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={vendorProfile.logoUrl} alt="Current logo" className="w-14 h-14 object-cover border border-zinc-200" />
              <div>
                <p className="text-sm font-semibold text-zinc-700">Current logo</p>
      ...
      <div>
        ...
        {vendorProfile.documents.length > 0 && (
          <div className="space-y-2 mb-3">
            {vendorProfile.documents.map((doc) => (
              <a
                ...
                className="flex items-center justify-between gap-3 px-4 py-2.5 bg-zinc-50 border border-zinc-200 hover:border-amber-300 transition-colors group"
              >
                <div className="flex items-center gap-2 min-w-0">
                  <FileText size={14} className="text-amber-500 shrink-0" />
                  <span className="text-xs text-zinc-700 truncate">{doc.documentType}</span>
                  {doc.verifiedAt && (
                ...
                <ExternalLink size={12} className="text-zinc-400 group-hover:text-amber-500 transition-colors shrink-0" />
              </a>
            ))}
          </div>
        )}

        {/* Newly added documents (pending submit) */}
        {newDocFiles.length > 0 && (
          <div className="space-y-2 mb-3">
            {newDocFiles.map((file, index) => (
              <div key={index} className="flex items-center justify-between gap-3 px-4 py-2.5 bg-amber-50 border border-amber-200">
                <div className="flex items-center gap-2 min-w-0">
                  <FileText size={14} className="text-amber-500 shrink-0" />
                  <span className="text-xs text-zinc-700 truncate">{file.name}</span>
                  <span className="text-xs text-zinc-400 shrink-0">({(file.size / 1024).toFixed(0)} KB)</span>
                </div>
                <button type="button" onClick={() => removeNewDoc(index)} className="text-zinc-400 hover:text-red-500 transition-colors shrink-0">
                  <X size={13} />
                </button>
              </div>
            ))}
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-zinc-200 text-zinc-500 hover:border-amber-400 hover:text-amber-500 bg-zinc-50 px-5 py-4 text-sm font-semibold transition-colors"
        >
          <Upload size={15} />
          Add new documents
        </button>
        <input ref={docsInputRef} type="file" multiple className="hidden" onChange={handleDocsChange} />
      </div>
                ...
                <div className="absolute z-50 left-0 right-0 top-full mt-0.5 bg-white border border-zinc-200 shadow-md">
                  {suggestions.map((feature) => (
                    ...
                    >
                      ...
                      <span className="text-sm text-zinc-700 leading-snug">{feature.properties.formatted}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          {showMap && mapPreviewUrl && (
            <div className="border border-zinc-200 overflow-hidden">
              <div className="px-3 py-2 bg-zinc-50 border-b border-zinc-200 flex items-center gap-2">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-zinc-500">Location Preview</span>
              </div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
