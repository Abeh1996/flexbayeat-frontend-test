...
export function ForgotPasswordForm({ onSuccess }: ForgotPasswordFormProps) {
  ...
  const { forgotPassword, isForgotLoading } = useAuthMutation();
  ...
  const handlePhone = (v: PhoneValues) => {
    forgotPassword({ phone: v.phone }, {
      onSuccess: () => onSuccess(v.phone),
    });
  };

  const handleEmail = (v: EmailValues) => {
    forgotPassword({ email: v.email }, {
      onSuccess: () => onSuccess(v.email),
    });
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-1.5">
        <label className={labelCls}>Send reset link via</label>
